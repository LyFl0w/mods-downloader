...
def setup_target_resourcepacks():
    ...
    for line in lines:
        ...
        if is_already_save(mod_name):
            add_to_list(get_id_save(mod_name), mod_name, "resourcepack")
            continue
        ...
        while len(r) == 0 and index + 50 <= 10_000:
            r = request(f'{curse_forge_api}/mods/search', params={'gameId' : minecraft_id, 'classId' : resourcepacks_categorie_id, 'filterText': mod_name,
                                                            'index': index, 'sortField': 1})
            if "data" not in r:
                break
        ...
        if isinstance(r, dict) or index + 50 > 10_000:
            print(f"resourcepack not found : {mod_name}")
            continue

        mod = r[0]
        mod_id = mod['id']
        mod_name = mod['name']
        curse_forge_link = "https://www.curseforge.com/minecraft/texture-packs/"+mod["slug"]

        if not is_already_save(mod_name):
            to_write.append(f'{mod_name}{spliter}{mod_id}{spliter}{curse_forge_link}')
        add_to_list(mod_id, mod_name, "resourcepack")
